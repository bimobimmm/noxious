...
#-------------------------------
#  Menjalankan addmrpt.sql
#-------------------------------
echo ""
echo "✅ Instance $ORACLE_SID dipilih. Menjalankan addmrpt.sql..."
echo "📌 Input format, Snap ID, dan nama file report di dalam SQL seperti biasa."
...
#-------------------------------
#  Deteksi dan pindahkan file report
#-------------------------------
echo ""
echo "🔎 Mendeteksi file ADDM report yang baru dibuat..."

# Temukan file terbaru (html/txt) di direktori sekarang
latest_file=$(ls -t *.html *.txt 2>/dev/null | head -n 1)

if [[ -n "$latest_file" ]]; then
  target_dir="/home/oracle/noxious@scripts/automation/toolsinventory/AADM@automation"
  mkdir -p "$target_dir"
